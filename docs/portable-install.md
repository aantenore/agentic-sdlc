...
Agentic SDLC 0.16.0 includes a self-contained Codex packaging. The repository root is the plugin root because it contains `.codex-plugin/plugin.json`; all manifest and agent-card paths are repository-relative.
...
| --- | --- | --- |
| `codex plugin list --available --json` | Installed entry is enabled and reports `0.16.0` | Rerun staging, add again, then open a new task |
